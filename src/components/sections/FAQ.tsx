    ...
    <section id="faq" className="relative overflow-hidden border-y border-white/[0.04] bg-black px-4 py-16 sm:px-6 sm:py-24">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
        ...
        >
          ...
          <div className="theme-card mt-8 hidden rounded-2xl border border-white/[0.09] bg-black/60 p-5 backdrop-blur-xl lg:block">
        ...
        <motion.div
          className="theme-faq-wrap overflow-hidden rounded-3xl border border-white/[0.09] bg-black/70 backdrop-blur-xl"
        ...
        >
          {FAQS.map((item, index) => (
            <details
              key={item.question}
              className="theme-faq-item group border-b border-white/[0.08] px-5 py-5 transition-colors last:border-b-0 open:bg-white/[0.035] sm:px-7"
              open={index === 0}
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-5 text-left text-base font-semibold text-text-primary sm:text-lg">
                ...
                <span className="theme-faq-toggle flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/[0.10] bg-white/[0.035] text-lg text-text-muted transition-all group-open:rotate-45 group-open:border-white/20 group-open:text-white">
                  +
                </span>
              </summary>
